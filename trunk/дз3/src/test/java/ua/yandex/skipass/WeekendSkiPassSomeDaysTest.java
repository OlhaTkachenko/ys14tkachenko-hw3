 ...
 * @author Ольга
 */
public class WeekendSkiPassSomeDaysTest {
    
    public WeekendSkiPassSomeDaysTest() {
    }
...
@Test
    public void testCheckPartOfDay() {
        ...
        WeekendSkiPassSomeDays instance = new WeekendSkiPassSomeDays(current, 0);
        when(current.partOfDay()).thenReturn(1);
        when(current.equalsWithoutHours(current)).thenReturn(true);
        when(current.dayOfWeek()).thenReturn(6);
     ...
     @Test
      public void testCheck() {
        ...
        Date current = mock(Date.class);
        when(current.copy()).thenReturn(current);
        WeekendSkiPassSomeDays instance = new WeekendSkiPassSomeDays(current,2);
        when(current.equalsWithoutHours(current)).thenReturn(true);
        when(current.dayOfWeek()).thenReturn(6);
